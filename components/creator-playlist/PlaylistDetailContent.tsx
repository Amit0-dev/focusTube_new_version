...
export default async function PlaylistDetailContent({ tab, playlistId }: { tab: string, playlistId: string }) {
  ...
  try {
    if (playlist) {
      userCount = await getJoinedUserCountOfCreatorPlaylist(playlist.id)
    }
  } catch (error) {
    console.error("Error: failed to fetch userCount", error)
  }
  ...
  return (
    <>
      {playlist && <div className="flex min-h-full gap-6">
        {/* Main content area */}
