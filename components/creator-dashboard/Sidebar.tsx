...
import { LayoutDashboard, ListVideo, Sparkles } from 'lucide-react';
...
export default function CreatorSidebar({
  ...
  function goToTab(nextTab: string) {
    ...
    router.replace(`/creator?${nextSearchParams.toString()}`);
    onCloseMobile();
  }
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            ...
            className="h-10 w-10 rounded-2xl bg-linear-to-br from-cyan-500/30 to-sky-400/20 ring-1 ring-white/10"
      ...
      <div className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-4">
        ...
        <div className="mt-1 text-xs text-white/60">
          Publish playlist spaces, watch audience growth, and manage imports in
          one place.
        </div>
      </div>
