...
function buildPlaylistSignals(playlist: Playlist) {
  const base = playlist.itemCount * 11 + playlist.title.length * 7;

  return {
    learners: Math.max(24, base + 32),
    sessions: Math.max(8, Math.round(base / 3) + 12),
    notes: Math.max(3, Math.round(playlist.itemCount / 2) + 2),
  };
}
...
export default function CreatorPlaylistCard({
  ...
  const signals = buildPlaylistSignals(playlist);

  const quickStats = [
    {
      icon: Users2,
      label: 'Learners',
      value: signals.learners,
    },
  ] as const;
